...
//			dbPRJDB->DefaultDatabase = (_bstr_t)(basDirectory::DBShareDir+_T("WorkPrj.mdb"));
		...
		if(dbSORT== NULL)
		{
			...
			dbSORT->Open((_bstr_t)("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + basDirectory::ProjectDBDir+_T("sort.mdb")), 
				"", "", adModeUnknown);
		}
		...
		if(dbTable== NULL)
		{
			...
			dbTable->Open((_bstr_t)("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + basDirectory::DBShareDir+_T("TableFormat.mdb")),
				"", "", adModeUnknown);
		}
		//20071015  10:22(end)
		//20071018(start)
		if(dbDSize== NULL)
		{
			...
			dbDSize->Open((_bstr_t)("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + basDirectory::DBShareDir+_T("DrawingSize.mdb")), 
				"", "", adModeUnknown);
		}
		...
		if(dbMaterial== NULL)
		{
			...
			dbMaterial->Open((_bstr_t)("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + basDirectory::ProjectDBDir+_T("Material.mdb")), 
				"", "", adModeUnknown);
		}
		...
		if(dbPHScode== NULL)
		{
			...
			dbPHScode->Open((_bstr_t)("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + basDirectory::ProjectDBDir+_T("PHScode.mdb")), "", "", adModeUnknown);
		}

		if(dbSACal== NULL)
		{
			dbSACal.CreateInstance(__uuidof(Connection));
			dbSACal->Open((_bstr_t)("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + basDirectory::ProjectDBDir+_T("SAStructureCal.mdb")), "", "", adModeUnknown);
		}
		//20071018(end)
...
void EDIBgbl::InitDBTBN(CString  &strSQL)
{
 ...
 void EDIBgbl::CloseAll_ConnectionPtr()
 {
 	try{
		...
		if(dbTable)
		{
			if(dbTable->State!=adStateClosed)
				dbTable->Close();
			dbTable.Release();
			dbTable=NULL;
		}
		if(dbDSize)
		{
			if(dbDSize->State!=adStateClosed)
				dbDSize->Close();
			dbDSize.Release();
			dbDSize=NULL;
		}
		if(dbMaterial)
		{
			if(dbMaterial->State!=adStateClosed)
				dbMaterial->Close();
			dbMaterial.Release();
			dbMaterial=NULL;
		}
		if(dbPHScode)
		{
			if(dbPHScode->State!=adStateClosed)
				dbPHScode->Close();
			dbPHScode.Release();
			dbPHScode=NULL;
		}
		if(dbSACal)
		{
			if(dbSACal->State!=adStateClosed)
				dbSACal->Close();
			dbSACal.Release();
			dbSACal=NULL;
		}
...
void EDIBgbl::GetSelPrjName()
{
	...
	try
	{
 		...
 		if(dbPRJDB)
		{
			...
			hr = rs->Open((_bstr_t)SQLx, _variant_t((IDispatch*)dbPRJDB,true), 
				adOpenDynamic, adLockOptimistic, adCmdText); 
			_variant_t v;
			if(!rs->adoEOF)
			{
	...
	try
	{
		...
		for(POSITION pos = sListTableName->GetHeadPosition(); pos != NULL; )
		{
			...
			if(StringInList(strTableName, dListTableName))
			{
				...
				sTDef->Open(_variant_t(strSql),(IDispatch*)sDB,adOpenStatic, adLockOptimistic,adCmdText);

				_RecordsetPtr dTDef(__uuidof(Recordset));
				strSql.Format("SELECT * FROM [%s]", strTableName);
				dTDef->Open(_variant_t(strSql),(IDispatch*)dDB,adOpenStatic, adLockOptimistic,adCmdText);
...
*/
	...
	try
	{
		...
		strConnect.Format(_T("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=%s;Jet OLEDB:Database Password=%s"),
			strSourceDB, strSourcePwd);
		sDB->Open((_bstr_t)strConnect, "", "", adModeUnknown);

		strConnect.Format(_T("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=%s;Jet OLEDB:Database Password=%s"),
			strDestDB, strDestPwd);
		dDB->Open((_bstr_t)strConnect, "", (_bstr_t)strDestPwd, adModeUnknown);
...
*/
		...
		drsTmp->Open(_variant_t(SQLx),(IDispatch*)dDb,adOpenDynamic, adLockOptimistic,adCmdText);
...
bool EDIBgbl::UpdateAllPrjDB()
{
	...
	try
	{
		...
		sRsEngin->Open(_variant_t(_T("Select * From Engin")),(IDispatch*)sDb,adOpenStatic, adLockOptimistic,adCmdText);
		sRsEngin->MoveLast();
		sRsEngin->MoveFirst();
...
*/
		...
		dRsVolume->Open(_variant_t(_T("Select * From Volume")),(IDispatch*)dDb,adOpenDynamic, adLockOptimistic,adCmdText);
		...
		rsJD->Open(_variant_t(_T("Select * From DesignStage")),(IDispatch*)tmpSort,adOpenStatic, adLockOptimistic,adCmdText);
		rsZY->Open(_variant_t(_T("Select * From Speciality")),(IDispatch*)tmpSort,adOpenStatic, adLockOptimistic,adCmdText);
		int stc,sti;
		sti=0;
		...
		while(!sRsEngin->adoEOF)
		{
			...
			for(i=0;i<vc;i++)
			{
				...
				hr = rsZY->Find((_bstr_t)strFind, 0, adSearchForward);
				if(!rsZY->adoEOF)
				{
				...
				hr = rsJD->Find((_bstr_t)strFind, 0, adSearchForward);
				if(!rsJD->adoEOF)
				{
				...
				sRsVolume->Open(_variant_t(strSQL),(IDispatch*)sDb,adOpenStatic, adLockOptimistic,adCmdText);
		...
		while (!sRS->adoEOF)
		{
			if (!strcmp((_bstr_t)sRS->Fields->GetItem("TABLE_TYPE")->Value,"TABLE"))
			{
				...
				if (!dRSTable->adoEOF)
				{
					...
					sRSTable->Open(_variant_t(SQLx), (IDispatch*)sDb,adOpenStatic, adLockOptimistic,adCmdText);
					pF = sRSTable->Fields;
...
bool EDIBgbl::ChangeColumnsToRows(_ConnectionPtr db, CString TblName, CString OutTblName,int iRowStart,int iRowCount)
{
	...
	try
	{
		...
		if(rs1->State == adStateOpen)
			rs1->Close();
		rs1->Open(_variant_t(SQLx),(IDispatch*)db,adOpenDynamic, adLockOptimistic,adCmdText);

		SQLx=_T("SELECT * FROM ") + TblName;
		if(rs->State == adStateOpen)
			rs->Close();
		rs->Open(_variant_t(SQLx),(IDispatch*)db,adOpenStatic, adLockOptimistic,adCmdText);
		if(rs->BOF || rs->BOF)
		{
		}
		else
