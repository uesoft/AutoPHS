...
COleSafeArray EDIBAcad::AcATTR ;//= _variant_t();
...
void EDIBAcad::SaveDwgAndExitACAD(_variant_t prompt, _variant_t DwgName)
{
}

void EDIBAcad::CloseAcadWin(_variant_t /*Optional*/ ExitMode)
{
}
...
void EDIBAcad::ModifyAttBG(float /*ByVal*/ ELvalue, _variant_t /*Optional*/ BGtype, _variant_t /*Optional*/ strTagStringOfBG)
{
}
...
void EDIBAcad::DrawTableExcel(long BillID, _RecordsetPtr rsText)
{
	...
	try
	{
		...
		SQLx = _T("SELECT * FROM TableINFO WHERE ID=") + ltos(BillID);
		rs->Open(_variant_t(SQLx),(IDispatch*)EDIBgbl::dbTable,
			adOpenStatic, adLockOptimistic,adCmdText);
		_variant_t vTmp;
				...
				if(ColTxt!=_T("") && iCol2>=iCol1)
				{
					try
					{
						...
						if( EDIBAcad::g_bCustomPicNo && str == _T("DRAWNO") )//如果为用户手工输入的图号
						{
							...
							rs1->Open(_variant_t(SQLstr),(IDispatch*)EDIBgbl::dbPRJDB,
								adOpenStatic, adLockOptimistic,adCmdText);
...
void EDIBAcad::AddData2rsTZG(_RecordsetPtr rs, long  iRecNo, long  zdjh, CString  FD, CString  sView, CString sBlkName, COleSafeArray& InsPnt)
{
	...
	{
      rs->AddNew();
      rs->put_Collect((_variant_t)_T("VolumeID"),_variant_t(EDIBgbl::SelVlmID));
      rs->put_Collect((_variant_t)_T("recno"),_variant_t(iRecNo));
      rs->put_Collect((_variant_t)_T("zdjh"),_variant_t(zdjh));
		CString sTmp;
      ...
      rs->put_Collect((_variant_t)_T("x"),_variant_t(va));
		ix=1;
		InsPnt.GetElement(&ix,&va);
      rs->put_Collect((_variant_t)_T("y"),_variant_t(va));
		ix=2;
		InsPnt.GetElement(&ix,&va);
      rs->put_Collect((_variant_t)_T("z"),_variant_t(va));
...
//		HeightOffset = 5;        //根部定位尺寸Y方向偏移值
		...
		rs->Open(_variant_t(strSql),(IDispatch*)EDIBgbl::dbPRJ,
			adOpenStatic, adLockOptimistic,adCmdText);
		if( rs->BOF || rs->adoEOF )
		{
			rc = 0;
...
// 		rs.Open(dbOpenSnapshot,SQLx );
		rs->Open(_variant_t(SQLx),(IDispatch*)EDIBgbl::dbPRJDB,adOpenStatic, adLockOptimistic,adCmdText);
		...
		rsTZG->Open(_variant_t(SQLx),(IDispatch*)EDIBgbl::dbPRJDB,adOpenDynamic, adLockOptimistic,adCmdText);
		//选择记录集rs以便查找图块名称
		...
		if(rs->State == adStateOpen)
			rs->Close();
		rs->Open(_variant_t(SQLx),(IDispatch*)EDIBgbl::dbPHScode,adOpenStatic, adLockOptimistic,adCmdText);
...
void EDIBAcad::DrawphsDataEdit()
{
	try
	{
		...
		rsY->Open(_variant_t(SQLx),(IDispatch*)EDIBgbl::dbPRJDB,adOpenDynamic, adLockOptimistic,adCmdText);
		...
		for(i = 0 ;i<c;i++)
		{
			...
			if(!rsY->BOF && !rsY->adoEOF)
			{
				...
				HRESULT hr = S_OK;
				hr = rsY->Find((_bstr_t)strSql, 0, adSearchForward);
				if(!rsY->adoEOF)
				{
					sTmp=vtos(::GetFields(rsY,_T("recno")));
					CString strFind;
					strFind = CString(_T("recno="))+sTmp;
					hr = FrmDataEDIT.m_Data1->Find((_bstr_t)strSql, 0, adSearchForward);
				}
			}
		}
...
void EDIBAcad::DrawTagByPickEntity(_variant_t /*Optional*/ pnt)
{
}
...
void EDIBAcad::DrawTagAll()
{
	...
	try
	{
		...
		if(!EDIBAcad::g_bDimNumber)
		{
			...
			rsRefZB->Open(_variant_t(SQLx),(IDispatch*)EDIBgbl::dbPRJDB,adOpenDynamic, adLockOptimistic,adCmdText);
			if( rsRefZB->adoEOF || rsRefZB->BOF )
			{
			}
			else
			{
				...
				while( i < 20 )
				{
					...
					HRESULT hr = S_OK;
					hr = rsRefZB->Find((_bstr_t)str, 0, adSearchForward);
					while( !rsRefZB->adoEOF )
					{
						rsRefZB->put_Collect((_variant_t)_T("seq"),_variant_t());
						rsRefZB->Update();
		...
		else
		{
			...
			else
			{
				...
				while(!rsTZG->adoEOF)
				{
					...
					hr = rsRefZB->Find((_bstr_t)strFind, 0, adSearchForward);
					if( !rsRefZB->adoEOF)
					{
...
//void EDIBAcad::VBSetXdata(Object ent, _variant_t /*ByVal*/ vbApp, _variant_t ParamArray xData())
//{
//}

//bool EDIBAcad::VBGetXdata(Object ent, CString /*ByVal*/ vbApp, int xdataType(), _variant_t xData())
//{
//}

//bool EDIBAcad::VBGetEXdata(Object ent, CString /*ByVal*/ vbApp, _variant_t /*Optional*/ IxdataType, _variant_t /*Optional*/ VxData)
//{
//}
...
CString EDIBAcad::GetDrawIDAndName(long lngSEQ, CString& strDrawName)
{
	...
	try
	{
		...
		rs->Open(_variant_t(SQLx),(IDispatch*)EDIBgbl::dbDSize,adOpenStatic, adLockOptimistic,adCmdText);
		if(rs->BOF || rs->adoEOF)
		{
			if(rs->State == adStateOpen)
				rs->Close();
