...
void CInPlaceList::OnSelchange() 
{
	...
	if(pGrid != NULL && pGrid->IsKindOf(RUNTIME_CLASS(CGridCtrl)))
	{
		CGridComboBoxCell* pCell=(CGridComboBoxCell*)pGrid->GetCell(this->m_nRow,this->m_nCol);
		if(pCell!=NULL && pCell->IsKindOf(RUNTIME_CLASS(CGridComboBoxCell)))
		{
			pCell->m_iCurSel=GetCurSel();
		}
	}
}
