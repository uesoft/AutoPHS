...
{
	float mvSumHeight =0;
	long iSEQofSPR, iSEQofPart, iChkCondition = 0;
	//	return true;
...
// 	{
	...
	float sngDim=0 , sngCSLen=0 , sngWeight =0, sngW=0 ;
		...
		_variant_t vTmp1,vTmp2,vTmp3,vTmp;
		_variant_t vBH;
		CString sTmp,sTmp2;
	_RecordsetPtr rs1;
	_RecordsetPtr rs3;
					...
					case iAttached: 
						...
						{	//连接件L15实际上是一块钢板
							...
							if(rsTmp->BOF && rsTmp->adoEOF)
							{
								//如果不存在这块钢板,加入它。
								/*rsTmp->AddNew();
								rsTmp->put_Collect((_variant_t)_T("CustomID"),_variant_t(_T("PS")));
								rsTmp->put_Collect((_variant_t)_T("BH"),_variant_t(sTmp));
								rsTmp->put_Collect((_variant_t)_T("size2"),_variant_t(modPHScal::gfPSThickness));
								rsTmp->put_Collect((_variant_t)_T("Weight"),_variant_t(fPSWeight));
								rsTmp->Update();*/
	...
	case iSALbraceFixG48:               
		if((Ptype[i]!=_T("SJ8")))
		{
			...
			else
			{
				while (!rsX->adoEOF)
				{
					...
					HRESULT hr = S_OK;
					strTmp = _T("BH=\'")+vtos(vTmp1)+_T("\'");
					hr = rsTmp->Find((_bstr_t)strTmp, 0, adSearchForward);
					if( !rsTmp->adoEOF)
					{
		...
		else
		{
			...
			{				
				...
				//rsTmp1->put_Collect((_variant_t)_T("CustomIDsa"),_variant_t(Ptype(i)));
				rsTmp1->put_Collect((_variant_t)_T("CustomIDsa"),_variant_t(sPartID));
				rsTmp1->put_Collect((_variant_t)_T("H"),_variant_t(modPHScal::PAfixH));
				rsTmp1->put_Collect((_variant_t)_T("gdw2"),_variant_t(modPHScal::gdw));
				rsTmp1->put_Collect((_variant_t)_T("tj"),_variant_t(modPHScal::t0));
				//许用应力单位kgf/cm2->kgf/mm2
				rsTmp1->put_Collect((_variant_t)_T("SIGMAjt"),_variant_t(Sigmat/100.0));
				rsTmp1->put_Collect((_variant_t)_T("SIGMAj20"),_variant_t(Sigma0/100.0));
				rsTmp1->put_Collect((_variant_t)_T("SIGMAj100"),_variant_t(Sigmat100/100.0));
				rsTmp1->Update();
				//以上一句加快更新，尽量防止锁定。
				...
				rsTmp1->put_Collect((_variant_t)_T("bb"),_variant_t(bb));
				//获得主型钢P1
				...
				do
				{
					...
					rsTmp1->put_Collect((_variant_t)_T("J1y"),_variant_t(J1y));
					//构件1槽钢惯性矩Iy,cm4->mm4
					rsTmp->get_Collect((_variant_t)_T("Iy"), &vTmp);
					float J1z=vtof(vTmp)*10000.0;
					rsTmp1->put_Collect((_variant_t)_T("J1z"),_variant_t(J1z));
					//构件1槽钢最小惯性矩,cm4->mm4
					rsTmp1->put_Collect((_variant_t)_T("Imin1"),_variant_t(J1z>J1y?J1y:J1z));
					//构件1槽钢抗弯矩Wx,cm3->mm3
					rsTmp->get_Collect((_variant_t)_T("Wx"), &vTmp);
					float W1y=vtof(vTmp)*1000.0;
					rsTmp1->put_Collect((_variant_t)_T("W1y"),_variant_t(W1y));
					//构件1槽钢抗弯矩Wy,cm3->mm3
					rsTmp->get_Collect((_variant_t)_T("Wy"), &vTmp);
					float W1z=vtof(vTmp)*1000.0;
					rsTmp1->put_Collect((_variant_t)_T("W1z"),_variant_t(W1z));
					//构件1槽钢截面积S,cm2->mm2
					rsTmp->get_Collect((_variant_t)_T("S"), &vTmp);
					float F1=vtof(vTmp)*100.0;
					rsTmp1->put_Collect((_variant_t)_T("F1"),_variant_t(F1));
					//构件1槽钢高度h,mm
					...
					rsTmp1->put_Collect((_variant_t)_T("J2y"),_variant_t(J2y));
					//构件2槽钢惯性矩Iy,cm4->mm4
					rsTmp->get_Collect((_variant_t)_T("Iy"), &vTmp);
					float J2z=vtof(vTmp)*10000.0;
					rsTmp1->put_Collect((_variant_t)_T("J2z"),_variant_t(J2z));
					//构件1槽钢最小惯性矩,cm4->mm4
					rsTmp1->put_Collect((_variant_t)_T("Imin2"),_variant_t(J2z>J2y?J2y:J2z));
					//构件2槽钢抗弯矩Wx,cm3->mm3
					rsTmp->get_Collect((_variant_t)_T("Wx"), &vTmp);
					float W2y=vtof(vTmp)*1000.0;
					rsTmp1->put_Collect((_variant_t)_T("W2y"),_variant_t(W2y));
					//构件2槽钢抗弯矩Wy,cm3->mm3
					rsTmp->get_Collect((_variant_t)_T("Wy"), &vTmp);
					float W2z=vtof(vTmp)*1000.0;
					rsTmp1->put_Collect((_variant_t)_T("W2z"),_variant_t(W2z));
					//构件2槽钢截面积S,cm2->mm2
					rsTmp->get_Collect((_variant_t)_T("S"), &vTmp);
					float F2=vtof(vTmp)*100.0;
					rsTmp1->put_Collect((_variant_t)_T("F2"),_variant_t(F2));
					//构件2槽钢高度h,mm
					rsTmp->get_Collect((_variant_t)_T("h"), &vTmp);
					float h2=vtof(vTmp);
					rsTmp1->put_Collect((_variant_t)_T("h2"),_variant_t(h2));
					//构件2槽钢腿宽b,mm
					rsTmp->get_Collect((_variant_t)_T("b"), &vTmp);
					float b2=vtof(vTmp);
					rsTmp1->put_Collect((_variant_t)_T("b2"),_variant_t(b2));
					
					//双槽钢净空距离g,mm
					float g=PL3-2*h1;
					rsTmp1->put_Collect((_variant_t)_T("g"),_variant_t(g));
					//双槽钢中心距B1,mm
					float B1=PL3-h1;
					//因为rsTmp1中存在字段b1,而Jet数据库字段名是不区分大小写的，故换B1->Bx,表中相应的公式B1已经替换。
					rsTmp1->put_Collect((_variant_t)_T("Bx"),_variant_t(B1));
					//更新数据准备计算
					rsTmp1->Update();					
					...
					for(i4=1;i4<=2;i4++)
					{
						...
						rsTmp1->put_Collect((_variant_t)_T("Px"),_variant_t(mfpx));
						rsTmp1->put_Collect((_variant_t)_T("Py"),_variant_t(mfpy));
						rsTmp1->put_Collect((_variant_t)_T("Pz"),_variant_t(mfpz));
						rsTmp1->put_Collect((_variant_t)_T("Mx"),_variant_t(mfmx));
						rsTmp1->put_Collect((_variant_t)_T("My"),_variant_t(mfmy));
						rsTmp1->put_Collect((_variant_t)_T("Mz"),_variant_t(mfmz));
						rsTmp1->Update();
						
						//构件1和构件2焊缝的抗弯矩,mm3
						HRESULT hr = S_OK;
						strTmp = _T("SEQ=1");
						hr = rsCal->Find((_bstr_t)strTmp, 0, adSearchForward);
						if( rsCal->adoEOF)
						...
						else
						{
							...
							{//几何数据，只计算一次，加快速度
								...
								{
									HRESULT hr = S_OK;
									strTmp = _T("Lamda>")+ftos(mfLamda1);
									hr = rs2->Find((_bstr_t)strTmp, 0, adSearchForward);
									if( rs2->adoEOF)
									{
									...
									else
									{
										...
										else
										{
											...
											rsTmp1->put_Collect((_variant_t)_T("Fi1"),_variant_t(mfFi1));																					
											rsTmp1->Update();
										}
									}
									//求构件2稳定折减系数Fi2
									strTmp = _T("Lamda>")+ftos(mfLamda2);
									hr = rs2->Find((_bstr_t)strTmp, 0, adSearchForward);
									if( rs2->adoEOF)
									{
										//计算的细长比%s超过%s库%s表中最大值。
									...
									else
									{
										...
										else
										{
											...
											rsTmp1->put_Collect((_variant_t)_T("Fi2"),_variant_t(mfFi2));																					
											rsTmp1->Update();
										}
									}
								}
						...
						rsTmp1->put_Collect((_variant_t)_T("Py1"),_variant_t(Py1r1));
						rsTmp1->put_Collect((_variant_t)_T("Py2"),_variant_t(Py1r2));
						rsTmp1->Update();
...
spZ1Z2:
	...
	strTmp = "recno="+ltos(iTmpRecno);
	hr = rsTZB->Find((_bstr_t)strTmp, 0, adSearchForward);
	rsTZB->put_Collect((_variant_t)_T("nth"),_variant_t((short)nth));//标准支吊架路数=1，包括对称双吊和共用根部双吊
	...
	if( vTmp1.vt==VT_NULL || vtos(vTmp1) == _T("") )
	{
		if( sPartID==_T("L7") || sPartID==_T("G12") || sPartID==_T("L7Dd") || sPartID==_T("G12Lc") )
		{
			...
			if( PtypeID[i - 1] == _T("L8") || PtypeID[i - 1] == _T("L8Dd") )
			{
				...
				rsTZB->put_Collect((_variant_t)_T("PreviousPartInsertLen"),_variant_t((long)vtoi(vTmp2)));
			}
			////////////////////////////////////////////
			if( PtypeID[i + 1] == _T("L8") || PtypeID[i + 1] == _T("L8Dd"))
			{
	...
	else
	{
			...
			if(sPartID != _T("D4") && sPartID != _T("D5") && sPartID != _T("D11"))
			{
				...
				else
				{
					...
					else
					{
						rsConnect->get_Collect((_variant_t)_T("CNTBExtLenFLD"), &vTmp2);
						tmpExtLenFD = vtos(vTmp2);
						rsTZB->put_Collect((_variant_t)_T("PreviousPartInsertLen"),_variant_t((long)tmpExtLenValue));
					}
				}
			}
			//////////////////////////////////////////////////////////////////////
		...
		else
		{
			if( sPartID == _T("L8") || sPartID == _T("L8Dd") )
			{
				...
				//				 rsConnect.GetFieldValue(_T("CNTBExtLenFLD"),vTmp1);
				//			 	 tmpExtLenFD = vtos(vTmp1);
				//				 rsTZB.SetFieldValue(_T("PreviousPartInsertLen"),_variant_t((long)tmpExtLenValue));
				//			 }
				////////////////////////////////////////////////////////////
			}
			else
	...
	if(i==0)
	{
		...
		if( modPHScal::gintParallelNum == 2 )
		{
			if( modPHScal::glClassID == iPAh )
			{
				...
				rsTZB->put_Collect((_variant_t)_T("sizeC"),_variant_t((float)modPHScal::DblHangerRodDist));
			}
			else
			{
	...
	rsTZB->put_Collect((_variant_t)_T("recno"),_variant_t((long)iSEQofPart));
	dbstr = dbstr + " recno= " + ltos(iSEQofPart);
	rsTZB->put_Collect((_variant_t)_T("VolumeID"),_variant_t(EDIBgbl::SelVlmID));
...
//	AfxMessageBox(dbstr);
	...
	rsTZB->put_Collect((_variant_t)_T("ClassID"),_variant_t((long)modPHScal::glClassID));
	//计算荷载写入结果表，便于校核计算
	rsTZB->put_Collect((_variant_t)_T("Pjs"),_variant_t((float)tmpSelPJG));
	//零件左视图第一路方向
	rsTZB->put_Collect((_variant_t)_T("fx"),_variant_t((long)mlfx));
	//零件y坐标方向
	rsTZB->put_Collect((_variant_t)_T("Rotation"),_variant_t((long)mlRot));
	if( sPartID == _T("LS") || sPartID == _T("LT") )
	{
		...
		rsTZB->put_Collect((_variant_t)_T("sizeDIM"),_variant_t((long)-(vtof(vTmp1) + modPHScal::dwB / 2)));
		//零件实际尺寸
		rsTZB->put_Collect((_variant_t)_T("sizeH"),_variant_t((double)-(vtof(vTmp1) + modPHScal::dwB / 2)));
	}
	...
	else if( sPartID == _T("DLR") )
	{
		...
		rsTZB->put_Collect((_variant_t)_T("sizeDIM"),_variant_t((long)(vtof(vTmp1)+ modPHScal::dwB / 2 - vtof(vTmp2))));
		//零件实际尺寸
		rsTZB->put_Collect((_variant_t)_T("sizeH"),_variant_t((double)(long)(vtof(vTmp1)+ modPHScal::dwB / 2 - vtof(vTmp2))));
	}
	else if( sPartID == _T("LM2") || sPartID == _T("LM3") || sPartID == _T("LM4") )
	{
		...
		rsX->get_Collect((_variant_t)_T("sizeH"), &vTmp1);
		rsTZB->put_Collect((_variant_t)_T("sizeDIM"),_variant_t((long)vtof(vTmp1)));
		//零件实际尺寸
		rsTZB->put_Collect((_variant_t)_T("sizeH"),_variant_t((double)vtoi(vTmp1)));
	}
	...
	else if(sPartID==_T("L15"))
	{
		...
		{
			//混凝土支墩，钢板埋入其中
			rsTZB->put_Collect((_variant_t)_T("sizeDIM"),_variant_t((long)0));
			//零件实际尺寸
			rsTZB->put_Collect((_variant_t)_T("sizeH"),_variant_t((double)0));
		}
		else
		{
			rsTZB->put_Collect((_variant_t)_T("sizeDIM"),_variant_t((long)modPHScal::gfPSThickness));
			//零件实际尺寸
			rsTZB->put_Collect((_variant_t)_T("sizeH"),_variant_t((double)modPHScal::gfPSThickness));
		}
	}
	else
	{
	...
	rsTZB->put_Collect((_variant_t)_T("CLnum"),_variant_t((long)iNumPart));
	rsTZB->put_Collect((_variant_t)_T("SEQ"),_variant_t((long)iSEQofPart));
	...
	if( sPartID == _T("T5") )//T5和LHG不会同时出现，因为弹簧和恒力弹簧不会同时出现
		modPHScal::g_fLHGWeight = vtof(vTmp1);
	rsTZB->put_Collect((_variant_t)_T("CLdz"),_variant_t(vtof(vTmp1)));
	rsTZB->get_Collect((_variant_t)_T("CLnum"), &vTmp2);
	rsTZB->put_Collect((_variant_t)_T("CLzz"),_variant_t(vtof(vTmp1)* vtof(vTmp2)));
		...
		if(i==0)
		{
			...
			if(modPHScal::gintParallelNum == 2 )
			{
				if(modPHScal::glClassID == iPAh )
				{
					...
					rsX->get_Collect((_variant_t)_T("sizeC"), &vTmp1);
					modPHScal::DblHangerRodDist =vtof(vTmp1);
					rsTZB->put_Collect((_variant_t)_T("sizeC"),_variant_t(modPHScal::DblHangerRodDist));
				}
				else
				{
					if(modPHScal::glClassID == iPAs )
					...
					else
					{
						...
						else
						{
							//其它类零件保存中心距，如T5,LHG等
							rsX->get_Collect((_variant_t)_T("sizeC"), &vTmp1);
							rsTZB->put_Collect((_variant_t)_T("sizeC"),_variant_t(vtof(vTmp1)));
						}
					}
				}
			}
	...
	rsTZB->put_Collect((_variant_t)_T("Index"),_variant_t((long)modPHScal::glIDIndex));
	rsTmpZB->get_Collect((_variant_t)_T("CustomID"), &vTmp1);
	...
	if( !rsID->adoEOF)
	{
	   ...
	   case iROD:
		   ...
		   rsTZB->put_Collect((_variant_t)_T("sizeC"),_variant_t(modPHScal::giDiameter));
		   //拉杆直径保存到数组，以便过程末尾查找附件时使用。主要处理附件记录位于拉杆之前的这种情况，所有拉杆始端的附件都是这种情况。
		   //sizeH置为0便于后面统计总长
		   rsTZB->put_Collect((_variant_t)_T("sizeH"),_variant_t((long)0));
	   ...
	   case iSPR:
		   {
			   ...
			   rsTZB->put_Collect((_variant_t)_T("SizeH"),_variant_t(sngDim));
			   rsTZB->put_Collect((_variant_t)_T("sizeDIM"),_variant_t(sngDim));
			   ...
			   rsTZB->put_Collect((_variant_t)_T("SpringNo"), _variant_t( (long)modPHScal::sSprInfo[iSEQofSPR].DH) );
			   rsTZB->put_Collect((_variant_t)_T("LugDia"), _variant_t(modPHScal::sSprInfo[iSEQofSPR].FiJ));
			   rsTZB->put_Collect((_variant_t)_T("HeatDisp"), _variant_t(modPHScal::sSprInfo[iSEQofSPR].HeatDisp));
			   rsTZB->put_Collect((_variant_t)_T("HAZ"), _variant_t(modPHScal::sSprInfo[iSEQofSPR].haz));
	...
	if( iAttachIncd == iAttachIncluded && i < C - 1 )
	{
		...
		for (j = 1;j<= 8;j++)
		{
			...
			if(vtos(vTmp1)!=_T("") )
			{
				...
				rsPartBoltNuts->put_Collect((_variant_t)_T("VolumeID"),_variant_t(EDIBgbl::SelVlmID));
				rsPartBoltNuts->put_Collect((_variant_t)_T("zdjh"),_variant_t((long)modPHScal::zdjh));
				rsPartBoltNuts->put_Collect((_variant_t)_T("recno"),_variant_t((long)iSEQofPart));

				//rsPartBoltNuts.SetFieldValue(_T("index"),_variant_t((long)iBoltsNuts));
				rsPartBoltNuts->put_Collect((_variant_t)_T("nth"),_variant_t((short)nth));//标准支吊架路数=1，包括对称双吊和共用根部双吊
				rsConnect->get_Collect((_variant_t)(_T("CntbF")+sj+_T("num")), &vTmp1);

				if( vTmp1.vt==VT_NULL)
				{
					//螺栓螺母数量=其后零件数量*1///
					rsPartBoltNuts->put_Collect((_variant_t)_T("CLnum"),_variant_t(iNumPart * iNum * iCntbNum));
				}
				else
				{
					//附件数量=实际用量,包含vtmp1值 为零的情况
					rsPartBoltNuts->put_Collect((_variant_t)_T("CLnum"),_variant_t(vtoi(vTmp1) * iNumPart * iNum * iCntbNum));
										//lfx 2005.5.20改 
				...
				if( ! bWithBoltsNuts )
				{
					//厂家不提供的螺栓螺母
					//赋给任意非空的数字，便于编号
					rsPartBoltNuts->put_Collect((_variant_t)_T("SEQ"),_variant_t((long) i));

					//lfx 2005.5.20改 
					//说明见上面//lfx 2005.5.20改 注释
					//原rsPartBoltNuts->put_Collect((_variant_t)_T("CLbz"),vnil);
					rsPartBoltNuts->put_Collect((_variant_t)_T("CLbz"),_variant_t(strClbz));  
					//End 2005.5.20改
				}
				else
				{
					rsPartBoltNuts->put_Collect((_variant_t)_T("SEQ"),vnil);
					rsPartBoltNuts->put_Collect((_variant_t)_T("CLnum"),_variant_t((long)0));
					CString str;
	...
	strTmp = _T("IsNull(recno) AND (bUserAdd is null OR bUserAdd=0)");
	hr = rsTZB->Find((_bstr_t)strTmp, 0, adSearchForward);
		if( rsTZB->BOF )			
		{
	...
	else
	{
		...
		if(  vtoi(vTmp1)== iGCement )
		{
			...
			rsTZB->put_Collect((_variant_t)_T("CLdz"),_variant_t((long)0));
			rsTZB->put_Collect((_variant_t)_T("CLzz"),_variant_t((long)0));
			mvarSATotalWeight = 0;
		}
		else
		{
			//型钢根部,赋给一非空的任意值
			rsTZB->put_Collect((_variant_t)_T("sizeDIM"),_variant_t(SACSHeight));
			rsTZB->get_Collect((_variant_t)_T("index"), &vTmp1);
			vTmp2=rsza->GetCollect(_T("bNotSumSA"));
			if( vtoi(vTmp1)== iSA && !vtob(vTmp2)){
				//根部材料要统计
				rsTZB->put_Collect((_variant_t)_T("SEQ"),_variant_t((long)i));
				rsTZB->put_Collect((_variant_t)_T("CLdz"),_variant_t(mvarSATotalWeight));
				rsTZB->get_Collect((_variant_t)_T("CLNum"), &vTmp1);
				rsTZB->put_Collect((_variant_t)_T("CLzz"),_variant_t(mvarSATotalWeight * vtof(vTmp1)));
			}
			else
			{
				//根部材料不要统计
				//不赋给根部编号，防止其写入零件明细表
				rsTZB->put_Collect((_variant_t)_T("SEQ"),vnil);
				rsTZB->put_Collect((_variant_t)_T("CLNum"),_variant_t((long)0));
				rsTZB->put_Collect((_variant_t)_T("CLdz"),_variant_t((long)0));
				rsTZB->put_Collect((_variant_t)_T("CLzz"),_variant_t((long)0));
			}
		}
		...
		//小荷载大管径管夹按荷载选择，螺栓的直径M1变小，管夹规格中必须注明此M1值
	{
		...
		HRESULT hr = S_OK;
		hr = rsTZB->Find((_bstr_t)strSQLTmp, 0, adSearchForward);
		if( !rsTZB->adoEOF)
		{
...
bool Cphs::doiG100(int j, _RecordsetPtr rsTZB, _RecordsetPtr rsX, _variant_t& vTmp1, CString& sTmp, 
				   ...
				   int nth, _variant_t& vnil)
{
	bool ret = true;
				   ...
				   //if( sTmp!=_T("") )			
			   {
				   if(modPHScal::glClassID == iSAh )
				   {
					   ...
					   rsTZB->put_Collect((_variant_t)_T("sizeH"),_variant_t((float)modPHScal::WidthA));
					   rsTZB->put_Collect((_variant_t)_T("sizeDIM"),_variant_t((float)modPHScal::WidthA));
				   }
				   else
				   {
					   //非直接吊G11,即槽钢
					   ...
					   if( sTmp!= _T("") )
					   {
						   ...
						   rsTZB->put_Collect((_variant_t)_T("LSpan"),_variant_t((float)modPHScal::Lspan));
						   rsTZB->put_Collect((_variant_t)_T("XBL1"),_variant_t((float)(modPHScal::gdw+fLenA)));//XBL1记录悬臂长度  Add by luorijin 2008.10.30
						   //暂时只要计算主型钢的长度
						   sngCSLen = modPHScal::CSLength(sBHFormat, sTmp, sBH);
					   }
					   ...
					   if( (modPHScal::glClassID==iSAbeam) || (modPHScal::glClassID==iG52_55 ) )
						   rsTZB->put_Collect((_variant_t)_T("T"),_variant_t((float)(modPHScal::Lspan - modPHScal::gdw)));
					   else
					   {
						   rsX->get_Collect((_variant_t)_T("A"), &vTmp1);
					   ...
					   rsTZB->put_Collect((_variant_t)_T("L1"),_variant_t((float)sngCSLen));
					   rsTZB->put_Collect((_variant_t)_T("GDW1"),_variant_t(modPHScal::gdw));
					   //更新原始数据表TZA中的根部主槽钢数量
		  ...
		  for (j = 1 ;j<= 12;j++)
		  {
			  ...
			  if( sTmp!=_T("") && vtoi(vTmp2)>0 )
			  {
						  ...
						  else
						  {
							  ...
							  if(tmpID==_T("F9") || tmpID==_T("F10"))
							  {
								  if(modPHScal::glClassID==iG56 || modPHScal::glClassID==iG57 || modPHScal::glClassID==iG52_55 || modPHScal::glClassID==iG51)
								  {
									  ...
									  HRESULT hr = S_OK;
									  CString strFind;
										strFind = _T("(CustomID)=\'") + tmpCustomID + _T("\'");
									  hr = rsID->Find((_bstr_t)strFind, 0, adSearchForward);
									  if( !rsID->adoEOF)
									  {
										  rsID->get_Collect((_variant_t)_T("BHFormat"), &vTmp1);
										  sBHFormat = vtos(vTmp1);
				...
				rsSAPart->put_Collect((_variant_t)_T("VolumeID"),_variant_t(EDIBgbl::SelVlmID));
				rsSAPart->put_Collect((_variant_t)_T("zdjh"),_variant_t((long)modPHScal::zdjh));
				//2002.05.26以后改写此段代码，以与phs.arx思路一致。
				//是根部附件,则写记录号,
				//以便区分根部附件与部件.也便于对零件编号。
				iSEQofPart++;
				rsSAPart->put_Collect((_variant_t)_T("recno"),_variant_t((long)iSEQofPart));
				//因为根部附件与根部的区分标志是IsSAPart字段是否=-1。
				rsSAPart->put_Collect((_variant_t)_T("IsSAPart"),_variant_t((long)-1));
				//rsSAPart->put_Collect((_variant_t)_T("Index")) = iSA
				rsSAPart->put_Collect((_variant_t)_T("nth"),_variant_t((short)nth));//标准支吊架路数=1，包括对称双吊和共用根部双吊
				rsSAPart->put_Collect((_variant_t)_T("ClassID"),_variant_t((long)mviSASSClassID));
				rsSAPart->put_Collect((_variant_t)_T("Index"),_variant_t((long)mviSASSIndex));
				rsTmp->get_Collect((_variant_t)_T("CustomID"), &vTmp1);
				...
				{				
					...
					//根部螺栓或螺杆的长度，
					rsSAPart->put_Collect((_variant_t)_T("L1"),_variant_t( sngCSLen));
				}
				else
				{
					...
					if(mviSASSIndex==iSectionSteel )
					{
						//根部型钢的长度，
						rsSAPart->put_Collect((_variant_t)_T("L1"),_variant_t( sngCSLen));
						//rsSAPart->put_Collect((_variant_t)_T("GDW1"),_variant_t(modPHScal::gdw));
				...
				rsSAPart->put_Collect((_variant_t)_T("CLdz"),_variant_t(sngWeight));
				rsX->get_Collect((_variant_t)(_T("Pnum")+sj), &vTmp1);
				rsSAPart->put_Collect((_variant_t)_T("CLzz"),_variant_t(sngW * vtof(vTmp1)));
				rsID->get_Collect((_variant_t)_T("Description"), &vTmp1);
  ...
  bool Cphs::doiCSPR(int j, _RecordsetPtr rsTZB, _RecordsetPtr rsX, _variant_t& vTmp1, CString& sTmp, 
	  _variant_t& vTmp2, CString& sTmp2, CString& sBHFormat, CString& sBH, float& sngCSLen, 
	  _RecordsetPtr rsza, CString& mvSAattachedCustomID, _RecordsetPtr rsTmp, CString& SQL1, 
	  float& sngWeight, long& mviSASSClassID, CString& tmpCustomID, float& tmpSize2, float& tmpSizeH,
	  float& sngW, long& mviSASSIndex, CString& mvsSASSMaterial, _variant_t& vBH, _RecordsetPtr rsSAPart, 
	  ...
	  CString SQLx;
			   ...
			   rsTZB->put_Collect((_variant_t)_T("T"),_variant_t((long)vtoi(vTmp1)));
			   ...
			   else
			   {
				   ...
				   rsTZB->put_Collect((_variant_t)_T("AllowedMinLugExt"),_variant_t(vtof(vTmp1)));//保存到数据库，以便CalLugLength函数使用
				   rsTmp->get_Collect((_variant_t)_T("size5"), &vTmp1);
				   rsTZB->put_Collect((_variant_t)_T("AllowedMaxLugExt"),_variant_t(vtof(vTmp1)));
			   }
			   ...
			   if( sPartID == _T("LHA") || sPartID == _T("LHB") || sPartID == _T("PHA")|| sPartID == _T("PHB"))
			   {
				   ...
				   rsTZB->put_Collect((_variant_t)_T("AllowedMaxLugExtUp"),_variant_t(vtof(vTmp1)));
				   rsTZB->put_Collect((_variant_t)_T("AllowedMinLugExtUp"),_variant_t(vtof(vTmp1)));
			   }
			  ...
			  /* else
			   ...
			   rsTZB->put_Collect((_variant_t)_T("GDW1"),_variant_t(tmpSelPJG4CSPRFiK / iNumPart));
			   rsTZB->put_Collect((_variant_t)_T("SpringNo"), _variant_t( (long)modPHScal::sSprInfo[iSEQofSPR].DH) );
			   rsTZB->put_Collect((_variant_t)_T("LugDia"), _variant_t(modPHScal::sSprInfo[iSEQofSPR].FiJ));
			   rsTZB->put_Collect((_variant_t)_T("HeatDisp"), _variant_t(modPHScal::sSprInfo[iSEQofSPR].HeatDisp));
