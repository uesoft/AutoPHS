...
DWORD CCalStructDlg::CalStruct(float * pResult,PCalInfo pCalInfo,BOOL bMakeTab,int iTabNo)
{
	...
	try
	{
		...
		hr = rsLreal.CreateInstance(__uuidof(Recordset));
		_variant_t vTemp;
		...
		if(1 <= m_lStructID && m_lStructID <= 4)
		{
			...
			rsVar->MoveFirst();
			strTmp = _T("[VarName]=\'h\'");
			hr = rsVar->Find((_bstr_t)strTmp, 0, adSearchForward);
			if( !rsVar->adoEOF)
			{
				rsVar->get_Collect((_variant_t)1L, &vTemp);
				h=vtod(vTemp);
			...
			hr = rsVar->Find((_bstr_t)strFind, 0, adSearchForward);
			if( !rsVar->adoEOF)
			{
				rsVar->get_Collect((_variant_t)1L, &vTemp);
				xs=vtod(vTemp);
			...
			EDIBgbl::dbSACal->Execute((_bstr_t)strSQL, NULL, adCmdText);
		...
		if(bCalFi)
		{
			...
			hr = rsVar->Find((_bstr_t)strTemp, 0, adSearchForward);
			if( !rsVar->adoEOF)
			{
				rsVar->get_Collect((_variant_t)1L, &vTemp);
				l=vtod(vTemp);
...
void CCalStructDlg::UpdatePjg(LPCTSTR lpszPjg)
{
	...
	try
	{
		...
		for(;!rs->adoEOF;rs->MoveNext())
		{
			...
			rs->put_Collect((_variant_t)1L, COleVariant(dValue));
			rs->Update();
		}
		rs->Close();
		rs.Release();
...
void CCalStructDlg::CalComLen(_RecordsetPtr rstheVar)
{
	...
	try
	{
		...
		if(rstheVar==NULL)
		{
			...
			rsVar->Open((LPCTSTR)strSQL,(IDispatch*)this->m_connSASCal,adOpenKeyset, adLockOptimistic,adCmdText);
		}
		else
		{
