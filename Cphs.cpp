...
//			rsID->Close();
		...
		hr = rsID->Find((_bstr_t)temp, 0, adSearchForward);
		if( rsID->adoEOF)
			return false;
		else
			return true;
...
long Cphs::SetPhsPATypeToListBox()
{
	...
	try
	{
		...
		{
			CString strMsg;
			strMsg.Format(_T("%s:%d, %s"), __FILE__, __LINE__, GetResStr(IDS_NoAnyRecordInPictureClipData));
			ShowMessage(strMsg);
			return 0;
		}
		else
...
long Cphs::SetPhsTypeToListBox()
{
	...
	try
	{
		...
		{
			//连续2个零件连接匹配
			//sSQL=CString(_T("SELECT * FROM connect WHERE (cntb)=\'"))
			//	+modPHScal::sFindID(Trim(Cntb)+_T("\'"));
			sSQL=CString(_T(" (cntb)=\'"))+modPHScal::sFindID(Trim(Cntb))+_T("\'");
		...
		else
		{
			...
			HRESULT hr = S_OK;
			hr = rsConnect->Find((_bstr_t)sSQL, 0, adSearchForward);
			if( rsConnect->adoEOF)
			{            
			/*sSQL=CString(_T("SELECT * FROM connect WHERE (cntb)=\'"))
			+modPHScal::sFindID(Trim(Cntb))+_T("\'");
		...
		do
		{
			...
			sSQL = _T("(ID)=\'")+tmp+_T("\'");
			HRESULT hr = S_OK;
			hr = rsID->Find((_bstr_t)sSQL, 0, adSearchForward);
			if( !rsID->adoEOF)
			{
				//找到相应的ID
...
CString Cphs::GetPhsAssembleName(long /*Optional*/ SampleID)
{
	...
	try
	{
		...
		for( i = 0 ;i<IC;i++)
		{
			...
			if(i == IC - 1)
				//根部
				SQLx = _T("(ID)=\'") + Ptype[i] + _T("\' AND index=0");
			else
			...
			hr = rsID->Find((_bstr_t)SQLx, 0, adSearchForward);
			if( !rsID->adoEOF)
			{
				rsID->get_Collect((_variant_t)_T("CustomID"), &vTmp);
				strTmp = vtos(vTmp);
...
long Cphs::GetphsStructIDsemiAuto()
{
	...
	try
	{
		...
		if(rs1->State != adStateOpen || rs1->adoEOF||rs1->BOF)
			return -1;
		rs1->MoveFirst();
...
long Cphs::SavephsAllStructToTZB(long& zdjh, int nth, int& /*Optional*/ AttachIncluded)
{
	...
	try
	{
		...
		//连接表，保存的是ID值
		if(rsConnect->State != adStateOpen)
		{
		...
		CString sTmp2;
		_variant_t v,v1,v2,mvClassID,mvCustomID;
		_variant_t vTmp;
		...
		for( i = 0 ;i<= iCount-1;i++)// - 1
		{
			...
			if (i< iCount-1)// Then
			{
				...
				HRESULT hr = S_OK;
				hr = rsConnect->Find((_bstr_t)strSql, 0, adSearchForward);
				if( !rsConnect->adoEOF)
				{
					...
					if (AttachIncluded == iAttachIncluded)
					{
						...
						for( j = 1 ;j<= 8;j++)
						{
							...
							if (vtos(v1) != _T("") )
							{
								k = k + 1;
								rsTmpZB->AddNew();
								rsTmpZB->put_Collect((_variant_t)_T("VolumeID"),_variant_t(EDIBgbl::SelVlmID));
...
bool Cphs::GetphsBHandSizesTest()
{
	...
	try{
		...
		CString SQLx,SQLx2;
		_variant_t v,v1,v2;
		if( rsTmpREF->State != adStateOpen || (rsTmpREF->adoEOF && rsTmpREF->BOF )){
		...
		if( ! rsTmpREF->adoEOF )
		{
			...
			if( rsTmpREF->GetRecordCount() >= 2 )
			{
				...
				if( !rsID->Find((_bstr_t)strSql, 0, adSearchForward))
				{
					//没找到CustomID,出错
					sTmp.Format(GetResStr(IDS_NoRecordFLDEqvThisValueInPictureClipData),_T("CustomID"),vtos(v));
					throw (sTmp);
			...
			for (i = 0 ;i< rsTmpREF->GetRecordCount();i++)
			{
				...
				else 
				{
					...
					if( i < rsTmpREF->GetRecordCount() - 1 )
					{
						rsID->get_Collect((_variant_t)_T("ID"), &v);
						tmpID1=vtos(v);
						/*rsID->Find((_bstr_t)CString(_T("(CustomID)=\'")) + sv + _T("\'"))*/
						rsID->Find((_bstr_t)CString(_T("(CustomID)=\'") + sv + _T("\'")), 0, adSearchForward);
						if( !rsID->adoEOF )
						{
							rsID->get_Collect((_variant_t)_T("ID"), &v);
							tmpID1 = vtos(v);
						...
						HRESULT hr = S_OK;
						rsConnect->MoveFirst();
						hr = rsConnect->Find((_bstr_t)strFind, 0, adSearchForward);
						if( rsConnect->adoEOF)
						{
				...
				case iPAh:
					...
					{
						//SQLx = _T("SELECT * FROM ") & tbnPA & _T(" WHERE Dw>= ") & modPHScal::dj * (1 - k * 0.01) & _T(" AND Dw<= ") & modPHScal::dj * (1 + k * 0.01) & _T(" AND Pmax>=") & tmpSelPJG / iNumPart & _T(" AND Tj>=") & tmpT0 & _T(" AND (CustomID)='") & Ptype(i) & _T("' ORDER BY Pmax,Dw,tj,Weight")
						...
						SQLx+=sTmp;
						SQLx+= _T(" AND Tj>=") + stmpT0 + _T(" AND (CustomID)=\'") + sv + _T("\' ORDER BY tj,Dw,Pmax,Weight");
						//Debug.Print SQLx
				...
				case iPAGS:
					for (k = 0 ;k<=(int) modPHScal::gnDW_delta;k++)
					{
						//SQLx = _T("SELECT * FROM ") & tbnPA & _T(" WHERE Dw>= ") & modPHScal::dj * (1 - k * 0.01) & _T(" AND Dw<= ") & modPHScal::dj * (1 + k * 0.01) & _T(" AND Pmax>=") & tmpSelPJG / iNumPart & _T(" AND Tj>=") & tmpT0 & _T(" AND (CustomID)='") & Ptype(i) & _T("' ORDER BY Dw,tj,Weight")
						...
						SQLx+=sTmp;
						SQLx+= (_T(" AND Tj>=") + stmpT0 + _T(" AND (CustomID)=\'") + sv + _T("\' ORDER BY tj,Dw,Weight"));
						if(rsX->State == adStateOpen)
							rsX->Close();
						rsX->Open((_bstr_t)SQLx,_variant_t((IDispatch*)modPHScal::dbZDJcrude,true), 
							adOpenStatic, adLockOptimistic, adCmdText); 
				...
				case iPALX:
					...
					//这些(烟风煤粉管道管部)与管径无关
					//SQLx = _T("SELECT * FROM ") & tbnPA & _T(" WHERE Pmax>=") & tmpSelPJG / iNumPart & _T(" AND Tj>=") & tmpT0 & _T(" AND (CustomID)='") & Ptype(i) & _T("' ORDER BY Pmax,tj,Weight")
					
					sTmp.Format(_T("%g"),tmpSelPJG / iNumPart);
					SQLx = CString(_T("SELECT * FROM ")) + modPHScal::tbnPA + _T(" WHERE Pmax>=") + sTmp + _T(" AND Tj>=") + stmpT0 + _T(" AND (CustomID)=\'") + sv + _T("\' ORDER BY tj,Pmax,Weight");
				...
				case iROD:
					...
					//}else {
					//  SQLx = _T("SELECT * FROM ") + tbnPART + _T(" WHERE ") + tmpSQL + _T(" Pmax>=") + tmpSelPJG / iNumPart * Iif((rsza(_T("ifLongVertPipe")), iNumPart, 1) + _T(" AND (CustomID)=//") + Ptype(i) + _T("// ORDER BY dh,Weight")
					//}
					break;
				case iConnected:
				case iAttached:
					sTmp.Format(_T("%g"),tmpSelPJG / iNumPart * (rsza->GetCollect(_T("ifLongVertPipe")).boolVal ? iNumPart : 1));
					SQLx =CString( _T("SELECT * FROM ")) + modPHScal::tbnPART + _T(" WHERE ") + tmpSQL + _T(" Pmax>=") + sTmp + _T(" AND (CustomID)=\'") + sv + _T("\' ORDER BY dh,Weight");
					break;
				case iSPR:
					//弹簧的吊杆直径孔自动加大时，需要取消直径孔检查
					SQLx = CString(_T("SELECT * FROM [")) + modPHScal::tbnSPRINGCrude + _T("] WHERE ") + (tmpSQL != _T("") ? (modPHScal::gbSPRAutoLugDia ? _T("") : tmpSQL) : _T("")) + _T(" dh>=0 AND (CustomID)=\'") + sv + _T("\' ORDER BY tj,Weight");
					break;
					//SQLx = _T("SELECT * FROM [") + tbnSPRINGCrude + _T("] WHERE ") + Iif((tmpSQL <> _T(""), Iif((gbSPRAutoLugDia, _T(""), tmpSQL), _T("")) + _T(" dh=") + modPHScal::sSprInfo[iSEQofSPR].DH + _T(" AND (CustomID)=//") + Ptype(i) + _T("// ORDER BY dh,Weight")
				case iCSPR:
					SQLx =CString( _T("SELECT * FROM [")) + modPHScal::tbnHDCrude + _T("] WHERE ") + tmpSQL  + _T(" dh>=0 AND (CustomID)=\'") + sv + _T("\' ORDER BY dh,Weight");
					break;
					//SQLx = _T("SELECT * FROM [") + tbnHDCrude + _T("] WHERE ") + Iif((tmpSQL <> _T(""), tmpSQL, _T("")) + _T(" dh=") + modPHScal::sSprInfo[iSEQofSPR].DH + _T(" AND (CustomID)=//") + Ptype(i) + _T("// ORDER BY dh,Weight")
				case iSAh:
					sTmp.Format(_T("%g"),tmpSelPJG / iNumPart * (rsza->GetCollect(_T("ifLongVertPipe")).boolVal ? iNumPart : 1)); 
					SQLx = CString(_T("SELECT * FROM ")) + modPHScal::tbnSA + _T(" WHERE ") + tmpSQL + SAfDPmax + _T(">=") + sTmp + _T(" AND (CustomID)=\'") + sv + _T("\' ORDER BY GDW1,") + SAfDPmax;
					break;
				case iSACantilever:
				case iSALbrace:
					...
					sTmp.Format(_T("%g"),tmpSelPJG / iNumPart * (rsza->GetCollect(_T("ifLongVertPipe")).boolVal ? iNumPart : 1));
					SQLx+=sTmp  + _T(" AND (CustomID)=\'") + sv + _T("\' ORDER BY GDW1,") + SAfDPmax;
					break;
				case iG51:
				case iG56:
				case iG57:
					...
					sTmp.Format(_T("%g"),tmpSelPJG / iNumPart* (rsza->GetCollect(_T("ifLongVertPipe")).boolVal ? iNumPart : 1));
					SQLx+=sTmp   + _T(" AND (CustomID)=\'") + sv + _T("\' ORDER BY GDW1,size2,") + SAfDPmax;
					break;
				case iSAbeam:			
				case iG52_55:
					sTmp.Format(_T("%g"),modPHScal::gdw1);
					if( iNumPart == 2 )
					{
						...
						SQLx+=sTmp2+ _T(" AND (CustomID)=\'") + sv + _T("\' ORDER BY GDW1,L1,") + SAfDPmax;
					}
					else
					{
						//一个根部梁承担所有荷载
				...
				case iSALbraceFixG48:
					...
					if(sv!=_T("SJ8"))
					{
						...
						SQLx = CString(_T("SELECT * FROM ")) + modPHScal::tbnSA + _T(" WHERE GDW1>= ") + sTmp + _T(" AND PmaxSF>=")
							+ ftos(sngSAfixP)+ _T(" AND (CustomID)=\'") + sv + _T("\' ORDER BY GDW1,PmaxSF");
					}
					else
					{
					...
					/*case iGCement
					//混凝土结构不必处理
					SQLx = _T("SELECT * FROM ") + tbnSA + _T(" WHERE (CustomID)=//") + Ptype(i) + _T("//");
					*/			   
				default:
	...
	try{
		...
		if( !rs1->adoEOF)
		{
				...
				//for (int i = 0 ;i<rs1->GetRecordCount();i++)
			{
				...
				HRESULT hr = S_OK;
				hr = rs1->Find((_bstr_t)sTmp, 0, adSearchForward);
				if (!rs1->adoEOF)
				{
...
long Cphs::CheckDuplicateREFRecordWhenAppend(int *ipCheckButton)   //LFX  2005.3.24  加
{
	...
	try
	{
		...
		else 
		{
			...
			sTmp.Format(GetResStr(IDS_ExistsThisSampleIDinSampleREF), EDIBgbl::GetDBName(EDIBgbl::dbSORT), DupRec);  //原
...
void Cphs::CheckAllCustomIDandIDinPhsStructureREF()
{
	...
	brsIDStatus=TRUE;
	  ...
	  CustomID=vtos(v);
	  if(rsID->Find((_bstr_t)CString(_T("(CustomID)=\'")) + CustomID + _T("\'")))
	  {
	  rsID->get_Collect((_variant_t)_T("ID"), &v);
	  ID=vtos(v);
	  if (ID== _T("") )
	  ...
	  ID=vtos(v);
	  if(rsID->Find((_bstr_t)CString(_T("(ID)=\'")) + ID + _T("\'")))
...
void Cphs::ChangeNameInphsStructureName()
{
	...
	std::vector<_variant_t> vecCustomID;
	std::vector<int> vecClassID;
	std::vector<_variant_t>::iterator iter;
	vecID.reserve(200);
...
//			ShowMessage(sTmp);
		...
		else
		{
			...
			while(!rs2->adoEOF)
			{
				...
				HRESULT hr = S_OK;
				hr = rs3->Find((_bstr_t)EDIBgbl::SQLx, 0, adSearchForward);
				if (rs3->adoEOF)
				{
					rs3->AddNew();
...
void Cphs::GetphsSEQ(_RecordsetPtr /*ByVal*/ rsza)
{
	...
	try
	{
		...
		EDIBgbl::dbPRJDB->Execute((_bstr_t)(_T("SELECT ") + sGROUPBY + _T(" INTO TmpCLgroup IN \'") + EDIBgbl::GetDBName(EDIBgbl::dbPRJ) + _T("\' FROM [") + EDIBgbl::Btype[EDIBgbl::TZB] + _T("] WHERE zdjh=-1")), NULL, adCmdText);
		...
		//排序、分组统计零件，
		EDIBgbl::dbPRJDB->Execute((_bstr_t)(_T("INSERT INTO TmpCLgroup IN \'") + EDIBgbl::GetDBName(EDIBgbl::dbPRJ) + _T("\' SELECT DISTINCT ") + sGROUPBY + _T(" FROM [") + EDIBgbl::Btype[EDIBgbl::TZB] + _T("] WHERE VolumeID=") + ltos(EDIBgbl::SelVlmID) + _T(" AND zdjh=") + ltos(modPHScal::zdjh) + _T(" AND NOT ISNULL(seq)  GROUP BY ") + sGROUPBY), NULL, adCmdText);
		...
		CString SQLx;
		//建立空表tmp2
		SQLx = _T("SELECT ") + sGROUPBY + _T(" INTO tmp2 IN \'") + EDIBgbl::GetDBName(EDIBgbl::dbPRJ) + _T("\' FROM [") + EDIBgbl::Btype[EDIBgbl::TZB] + _T("] WHERE zdjh=-1");
		EDIBgbl::dbPRJDB->Execute((_bstr_t)SQLx, NULL, adCmdText);
		...
		{
			//如果结构表没有任何零件
			tmpStr.Format(GetResStr(IDS_NoRecordInTZB),EDIBgbl::GetDBName(EDIBgbl::dbPRJDB),EDIBgbl::Btype[EDIBgbl::TZB],
		...
		while(!rsTmpZB->adoEOF)
		{
			...
			//SQLx = _T("SELECT seq FROM tmpCLgroup INNER JOIN [") + EDIBgbl::TBNSelPrjSpec + EDIBgbl::Btype[EDIBgbl::TZB] + _T("] AS tbn ON  WHERE ISNULL(seq) AND (CLgg)=\'")
...
void Cphs::GetphsSumBom()
{
	...
	try
	{
		...
		else
		{
			while(!rs->adoEOF)
			{
				...
				else
				{
					...
					rsCL->put_Collect((_variant_t)_T("CLnum"),_variant_t(vtof(v)*vtof(vTmp1)));
					rsCL->Update();
				}
				rsza->Close();
				rs->MoveNext();
		...
		EDIBgbl::dbPRJ->Execute((_bstr_t)(_T("SELECT ") + sGROUPBY + _T(",sum(CLnum) as CLnums,CLnums*CLdz as CLzz INTO TmpCLgroup IN \'") + EDIBgbl::GetDBName(EDIBgbl::dbPRJ) + _T("\' FROM TmpTCL GROUP BY ") + sGROUPBY), NULL, adCmdText);
		...
		EDIBgbl::dbPRJ->Execute((_bstr_t)(_T("INSERT INTO [") + EDIBgbl::Btype[EDIBgbl::TCL] + _T("] IN \'") + EDIBgbl::GetDBName(EDIBgbl::dbPRJDB) + _T("\' SELECT * FROM TmpTCL")), NULL, adCmdText);
		//将恒力弹簧的内容拷贝到TmpCSPR表
		...
		else
		{
			...
			while(!rs->adoEOF)
			{
				...
				else
				{
					...
					rsCL->put_Collect((_variant_t)_T("CLdz"),_variant_t(vtof(v) * vtof(vTmp1)/1000.0)); //拉杆1m单重保存在GDW1字段
					rsCL->put_Collect((_variant_t)_T("CLnum"),_variant_t(sTmp));
					rsCL->put_Collect((_variant_t)_T("CLzz"),_variant_t(sngSumWeight));
					rsCL->Update();
				}
				rsza->Close();
				rs->MoveNext();
		...
		//将临时材料表的内容拷贝到正式材料表TCL
		//EDIBgbl::dbPRJ->Execute((_bstr_t)_T("INSERT INTO [") + EDIBgbl::TBNSelPrjSpec + EDIBgbl::Btype[EDIBgbl::TCL] + _T("] IN \'") + EDIBgbl::dbPRJDB) + _T("\' SELECT * FROM TmpTCL")
		//删除临时表
		RsDeleteAll(rsCL);
		//更新临时材料表数据记对象
				...
				else
				{
					...
					if (!rsID->adoEOF)
					{
						...
						if( (vtoi(GetFields(this->rsID,_T("Index")))==iSectionSteel) )
						{
							...
							rsCL->put_Collect((_variant_t)_T("CLdz"),_variant_t(vtof(GetFields(rs,_T("CLdz"))) * m_iL1/1000.0));
							rsCL->put_Collect((_variant_t)_T("CLnum"),_variant_t(sTmp));
							rsCL->put_Collect((_variant_t)_T("CLzz"),_variant_t(sngSumWeight));
							rsCL->put_Collect((_variant_t)_T("CLbz"),GetFields(rs,_T("CLbz")));
							rsCL->Update();
			...
			else
			{
				while(!rs->adoEOF)
				{
					...
					else
					{
						...
						rsCL->put_Collect((_variant_t)_T("CLnum"),_variant_t(vtof(GetFields(rsza,_T("Gnum"))) * vtof(GetFields(rsza,_T("num")))));
...
Cphs::Cphs()
{
	...
	rsConnect.CreateInstance(__uuidof(Recordset));
	rsUnCheckedType.CreateInstance(__uuidof(Recordset));
}
...
void Cphs::InitListRs()
{
	...
	try
	{
		...
		if(rsUnCheckedType->State != adStateOpen)
		{
...
void Cphs::CloseRecordsets()
{
	...
	try
	{
		if((rsConnect != NULL) && (rsConnect->State == adStateOpen))
			rsConnect->Close();
...
void Cphs::GetBoltsNutsAndAttachmentsCLgg(int nth )
{
	...
	try
	{
		...
		for( i = 0;i< IC;i++)
		{
			//this->rsID.FindFirst _T("(CustomID)=//") + RsTZB.Fields(_T("CustomID")) + _T("//")
		...
		for (i = 0 ;i< IC;i++)
		{
			...
			if( NewPtypeClassID[i] == iROD )
			{
				...
				if( PtypeIndex[lngCurrentPartNo - 1] == iPA || Ptype[lngCurrentPartNo - 1] == _T("T3A") )
				{
					...
					rsTmpZB->put_Collect((_variant_t)_T("A"),_variant_t(vtof(vTmp)));
					//同时将拉杆伸出段最小长度保存在字段C，便于计算拉杆长度函数CalLugLength处理
					rsTmpZB->put_Collect((_variant_t)_T("C"),_variant_t(vtof(vTmp)));
					rsTmpZB->Update();
					rsTmp->Close();
				}
				/*
				...
				*/
				if( PtypeIndex[lngCurrentPartNo + 1] == iSA || Ptype[lngCurrentPartNo + 1] == _T("T3") )
				{
					...
					rsTmpZB->put_Collect((_variant_t)_T("B"),_variant_t(vtof(vTmp)));
					//同时将拉杆伸出段最小长度保存在字段C，便于计算拉杆长度函数CalLugLength处理
					rsTmpZB->put_Collect((_variant_t)_T("C"),_variant_t(vtof(vTmp)));
					rsTmpZB->Update();
					rsTmp->Close();
							...
							{  //miSEQofCSPR
									...
									|| Ptype[lngCurrentPartNo]==_T("LHC"))
								{
									...
									if( rsTmp->adoEOF && rsTmp->BOF )
									{
										...
										{
											strTmp.Format(GetResStr(IDS_NotClearCheckAutoApplyCSPRLugDia),EDIBgbl::GetDBName(modPHScal::dbZDJcrude),modPHScal::tbnBoltsSurplusLength,SQLx);
											throw strTmp;
										}
									}
									//if( True ){
								...
								if( bCalBoltsDiaOfCSPR )
								{
									...
									if( rsX->adoEOF && rsX->BOF )
									{
										...
										rsTmpZB->get_Collect((_variant_t)_T("CustomID"), &vTmp);
										strTmp.Format(GetResStr(IDS_NoFoundAttachInZDJCrude),EDIBgbl::GetDBName(modPHScal::dbZDJcrude),SQLx,vtos(vTmp));
										ShowMessage(strTmp);
									}
									else
									{
										...
										if( NewPtypeID[i] == _T("F10") || NewPtypeID[i] == _T("F9") )
										{
											...
											else
											{
												...
												rsTmpZB->put_Collect((_variant_t)_T("CLdz"),_variant_t(vtof(vTmp1) / 1000 * LenBolts));
												rsTmpZB->put_Collect((_variant_t)_T("CLzz"),_variant_t(vtof(vTmp1) / 1000 * LenBolts * vtof(vTmp2)));
												if(NewPtypeID[i] == _T("F4"))
												{
													//一般可能使用45钢
										...
										else
										{
											...
											rsTmpZB->put_Collect((_variant_t)_T("CLzz"),_variant_t(vtof(vTmp1)* vtof(vTmp2)));
											if(NewPtypeID[i] == _T("F4"))
											{
												//一般可能使用45钢
												rsX->get_Collect((_variant_t)_T("Material"), &vTmp1);
					...
					else
					{
						...
						else
						{
							if( bFoundG12G13 && NewPtypeIndex[i] == iSA ){
								...
								rsTmpZB->put_Collect((_variant_t)_T("sizeH"),_variant_t(SACSHeight));
								rsTmpZB->Update();
							}
						}
					}
		...
		rsTmpZB->put_Collect((_variant_t)_T("CLdz"),_variant_t(mvarSATotalWeight));
		rsTmpZB->get_Collect((_variant_t)_T("CLnum"), &vTmp);
		rsTmpZB->put_Collect((_variant_t)_T("CLzz"),_variant_t(mvarSATotalWeight * vtof(vTmp)));
		rsTmpZB->Update();
