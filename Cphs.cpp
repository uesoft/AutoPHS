...
long Cphs::SavephsAllStructToTZB(long& zdjh, int nth, int& /*Optional*/ AttachIncluded)
{
	...
	try
	{
		...
		for( i = 0 ;i<= iCount-1;i++)// - 1
		{
			...
			{
				//取得下一次的开头零件
				CString strSql = (_T("trim(cntb)=\'") + (SampleArray[i]) + _T("\' AND (cnte)=\'") + (SampleArray[i+1]) + _T("\'"));
				HRESULT hr = S_OK;
...
bool Cphs::GetphsBHandSizesTest()
{
	...
	try{
		...
		if( ! rsTmpREF->adoEOF )
		{
			...
			HRESULT hr = S_OK;
			CString strSql;
			strSql = _T("trim(CustomID)=\'") + vtos(v) + _T("\'");
			hr = rsID->Find((_bstr_t)strSql, 0, adSearchForward);
			if( !rsID->adoEOF)
			{
				//出错
			...
			for (i = 0 ;i< rsTmpREF->GetRecordCount();i++)
			{
				...
				else 
				{
					...
					if( i < rsTmpREF->GetRecordCount() - 1 )
					{
						...
						CString strFind;
						strFind = CString(_T("trim(cntb)=\'") + tmpID0 + _T("\' AND (cnte)=\'") + tmpID1 + _T("\'"));
						HRESULT hr = S_OK;
						rsConnect->MoveFirst();
		...
		else
		{
			...
			while(!rs->adoEOF)
			{
				...
				else
				{
					...
					CString strFind;
					strFind = _T("trim(CustomID)=\'") + vtos(GetFields(rs,_T("CustomID"))) + _T("\'");
					HRESULT hr = S_OK;
					hr = rsID->Find((_bstr_t)strFind, 0, adSearchForward);
					if (!rsID->adoEOF)
					{
