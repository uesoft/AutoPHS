...
public:
	...
	static void EDIBAcad::DrawTableExcel(long BillID, CDaoRecordset& rsText);
	...
	static long GetTableHeaderBlockAttributes(CDaoRecordset& rs, bool  &bATTBEGIN, long &NPage,long& iNPage,long& SPage, CADatt* & BlkAtt,int iTabIndex=1/*默认为"材料汇总表"(即：EDIBgbl::TCL的值*/);
    static void DrawTableACAD(CCadPoint& pB, long BillID, double sngRotAng, CDaoRecordset& rsText, long lRowsPerPages=0, float fBlkScale=1.0f, LPCTSTR Layer=_T("bom"), LPCTSTR  Style=_T("hz"),LPCTSTR lpszColTxtFmt=_T("%g"),int iAlign=0);
    ...
    static void AddData2rsTZG(CDaoRecordset& rs, long  iRecNo, long  zdjh, CString  FD, CString  sView, CString sBlkName, COleSafeArray& InsPnt);
    static void DrawPhsAssemble(CDaoRecordset& rsTZB, long iView);
    static void DrawphsZUZI(CDaoRecordset& rsTZB, CCadPoint& InsPnt, long iView = iViewX);
    ...
    static void DrawTag(CDaoRecordset& rsTZG, int  iRecNo, CCadPoint& p0, CCadPoint& p1, long  iSEQ, CString  sLine="LINE", CString  sLayer="Tag");
