...
COleVariant modPHScal::sFindBlkPosFLD(CString  sFLD, CString  dFLD, CString  sID)
{
   ...
   try
   {
// 	    rs(&EDIBgbl::dbPHScode);//"dbSORT" 改为 "dbPHScode"
	   _RecordsetPtr rs;
...
// 	rsX.Open(dbOpenDynaset,_T("SELECT * FROM phsManuSPRING ORDER BY [Observation],[standard] ASC"));//,(IDispatch*)EDIBgbl::dbSORT,adOpenStatic,adLockOptimistic,adCmdText);
	...
	{
		  _variant_t vTmp;
		  rsX->Find((_bstr_t)(CString(_T("Trim(standard)=\'"))+gsPhsSPRINGSel+_T("\'")), 0, adSearchForward, vTmp);
		  if( !rsX->adoEOF)
		{
			rsX->MoveFirst();
			inttbExist=2;
...
//	   rsX.Open(dbOpenSnapshot,_T("SELECT * FROM phsManuConstantSPRING ORDER BY [Observation],[standard] ASC"));//),(IDispatch*)EDIBgbl::dbSORT,adOpenStatic,adLockOptimistic,adCmdText);
	   rsX->Open(_variant_t(SQLx),(IDispatch*)EDIBgbl::dbPRJDB,adOpenForwardOnly,adLockReadOnly,adCmdText);
	   ...
	   if(gsPhsConstantSpringSel != _T("") ){
		   _variant_t vTmp;
		   rsX->Find((_bstr_t)(CString(_T("Trim(standard)=\'"))+gsPhsConstantSpringSel+_T("\'")), 0, adSearchForward, vTmp);
		   if( !rsX->adoEOF)
		   {			
			   rsX->MoveFirst();
				inttbExist=2;
...
void modPHScal::ImportDataFromZdjCrude(CString  strFN, bool  bReplacement, bool & mbSPECchanged)
{
	...
	try
	{
		...
		{
// 			db.Open(strFN,FALSE,FALSE,CString(_T(";pwd="))+ ModEncrypt::gstrDBProductPassWord);
			CString ConnectionString="Provider=Microsoft.Jet.OLEDB.4.0;Persist Security Info=False;Data Source=" + strFN;
			db->Open((_bstr_t)ConnectionString, "", (_bstr_t)ModEncrypt::gstrDBProductPassWord, adConnectUnspecified);
			//取出管理表的名称
// 			rs.m_pDatabase=&EDIBgbl::dbSORT;
// 			rs.Open(dbOpenSnapshot,_T("SELECT * FROM phsManu ORDER BY seq"));
...
// 		  rsX.Open(dbOpenDynaset,SQL1x);
		  ...
		  if( rsX->BOF&& rsX->adoEOF)// Then
		  {
			 ...
			 //开始添加目录信息
			 rsX->AddNew();
			 rsX->put_Collect((_variant_t)_T("SEQ"),COleVariant((long)(zdjh + glDrawNoStartNumber - 1)));
			 rsX->put_Collect((_variant_t)_T("Pages"),COleVariant((long)1));
			 rsX->put_Collect((_variant_t)_T("VolumeID"),COleVariant(EDIBgbl::SelVlmID));
			 rsX->put_Collect((_variant_t)_T("DrawNo"),STR_VAR(sTmpDrawNo));
			 rsX->put_Collect((_variant_t)_T("DrawNa") ,STR_VAR( sTmpDrawNa));
			 rsX->put_Collect((_variant_t)_T("zdjh"),COleVariant((long)zdjh));
...
// 		rs1.Open(dbOpenDynaset,SQLx);
		...
		if(zdjh!=0)
		{
			...
			rs1->put_Collect((_variant_t)_T("VolumeID"),COleVariant(EDIBgbl::SelVlmID));
			...
			if(EDIBAcad::g_bHzabs)
				rs1->put_Collect((_variant_t)_T("paz1"),COleVariant(fabs(vtof(rsza->GetCollect(_T("paz1"))) * (float)UnitF)));
			else
				rs1->put_Collect((_variant_t)_T("paz1"),COleVariant(vtof(rsza->GetCollect(_T("paz1"))) * (float)UnitF));
			rs1->put_Collect((_variant_t)_T("pgz1"),COleVariant(vtof(rsza->GetCollect(_T("pgz1"))) * (float)UnitF));
			rs1->put_Collect((_variant_t)_T("pjg1"),COleVariant(vtof(rsza->GetCollect(_T("pjg1"))) * (float)UnitF));
			rs1->put_Collect((_variant_t)_T("xr1"),rsza->GetCollect(_T("xr1")));
			vTmp=rsza->GetCollect(_T("yr1"));
			float fyr1=(int(vtof(vTmp)*10))/10.0;
			rs1->put_Collect((_variant_t)_T("yr1"),COleVariant(fyr1));
...
//					rs1.Edit();
					if( b1st ){
						...
						rs1->put_Collect((_variant_t)_T("PAweight"), COleVariant(vtof(GetFields(rs1,_T("PAweight"))) +vtof(GetFields(rs,_T("CLzz")))));
					}else{
						...
						rs1->put_Collect((_variant_t)_T("PAweight"),COleVariant(vtof(GetFields(rs,_T("CLzz")))));
						//是第一个，设置标志。
						b1st = true;
					}
					rs1->Update();
...
// 					rs1.Edit();
					...
					{						
						...
						rs1->put_Collect((_variant_t)_T("SPRweight"), COleVariant(vtof(GetFields(rs1,_T("SPRweight"))) +vtof(GetFields(rs,_T("CLzz1")))));
					...
					{						
						...
						rs1->put_Collect((_variant_t)_T("SPRweight"),COleVariant(vtof(GetFields(rs,_T("CLzz1")))));
						//对于弹簧，函数GetPhsBHandSizes已经将相对安装压缩值写到TZB的GDW1字段，以便在函数PhsYLBmake输出一览表使用
						if( vtoi(GetFields(rs,_T("ClassID"))) == iSPR ) 
						{
							sTmp.Format(_T("%g"),int((sSprInfo[iSEQofSPR].haz + sSprInfo[iSEQofSPR].Hpcmprs)*10)/10.0);
...
// 					rs1.Edit();
					if( b1st ){
						...
						rs1->put_Collect((_variant_t)_T("RODweight"),COleVariant(vtof(GetFields(rs1,_T("RODweight"))) +vtof(GetFields(rs,_T("CLzz1")))));
					}else{
						...
						rs1->put_Collect((_variant_t)_T("RODweight"),COleVariant(vtof(GetFields(rs,_T("CLzz1")))));
						//是第一个，设置标志。
						b1st = true;
					}
					rs1->Update();
...
// 					rs1.Edit();
					if( b1st ){
						...
						rs1->put_Collect((_variant_t)_T("CntPartweight"),COleVariant(vtof(GetFields(rs1,_T("CntPartweight")))+vtof(GetFields(rs,_T("CLzz1")))));
					}else{
						...
						//是第一个，设置标志。
						rs1->put_Collect((_variant_t)_T("CntPartweight"),COleVariant(vtof(GetFields(rs,_T("CLzz1")))));
						b1st = true;
					}
					rs1->Update();
					rs->MoveNext();
...
// 					rs1.Edit();
					if( b1st ){
						...
						rs1->put_Collect((_variant_t)_T("Attachmentweight"),COleVariant(vtof(GetFields(rs1,_T("Attachmentweight")))+vtof(GetFields(rs,_T("CLzz1")))));
						...
						rs1->put_Collect((_variant_t)_T("Attachmentweight1"),COleVariant(vtof(GetFields(rs1,_T("Attachmentweight1")))+vtof(GetFields(rs,_T("CLzz1")))));
					}else{
						...
						//是第一个，设置标志。
						rs1->put_Collect((_variant_t)_T("Attachmentweight"),COleVariant(vtof(GetFields(rs,_T("CLzz1")))));
						...
						//是第一个，设置标志。
						rs1->put_Collect((_variant_t)_T("Attachmentweight1"),COleVariant(vtof(GetFields(rs,_T("CLzz1")))));
						b1st = true;
					}
					rs1->Update();
					rs->MoveNext();
...
// 					rs1.Edit();
					if( b1st ){
						...
						rs1->put_Collect((_variant_t)_T("Attachmentweight"),COleVariant(vtof(GetFields(rs1,_T("Attachmentweight")))+vtof(GetFields(rs,_T("CLzz")))));
					}else{
						...
						rs1->put_Collect((_variant_t)_T("Attachmentweight"),COleVariant(vtof(GetFields(rs,_T("CLzz")))));
						//是第一个，设置标志。
						b1st = true;
					}
					rs1->Update();
...
// 			rs.m_pDatabase=&EDIBgbl::dbPRJDB; rs.Open(dbOpenDynaset,SQLx);
			...
			if( !rs->BOF && !rs->adoEOF ){
				...
				while( !rs->adoEOF){
// 					rs1.Edit();
					rs1->put_Collect((_variant_t)_T("XBL1"),COleVariant(vtof(GetFields(rs,_T("XBL1")))));//悬臂长度，跟根部绑定 Add by luorijin 2008.10.30
					if( b1st ){
						...
						rs1->put_Collect((_variant_t)_T("SAweight"), COleVariant(vtof(GetFields(rs1,_T("SAweight"))) +vtof(GetFields(rs,_T("CLzz")))));
					}else{
						...
						rs1->put_Collect((_variant_t)_T("SAweight"),COleVariant(vtof(GetFields(rs,_T("CLzz")))));
						//是第一个，设置标志。
						b1st = true;
					}
					rs1->Update();
	   ...
	   rsTZB->Open((_bstr_t)strSQL, _variant_t((IDispatch*)EDIBgbl::dbPRJDB,true), 
		...
		if(!(rsTZB->BOF && rsTZB->adoEOF))
		{
			...
			else
			{
				while(!rsTZB->adoEOF)
				{
					if(!(rsTZB->adoEOF || rsTZB->BOF )) rsTZB->Delete(adAffectCurrent);
					if(!rsTZB->BOF )
					{	
						rsTZB->MoveFirst();					
					}
...
//				    rsTZB.Edit(); 
					rsTZB->put_Collect((_variant_t)_T("size"),COleVariant((float)(bA + bB)));
					rsTZB->Update();
				}//3
				else 
				{
//					rsTZB.Edit(); 
					rsTZB->put_Collect((_variant_t)_T("size"),COleVariant(3.0));
					rsTZB->Update();
				}
				rsTZB->MoveNext();
			}//4
...
//					    rsTZB.Edit();	
						rsTZB->put_Collect((_variant_t)_T("A"),COleVariant((float)n));
						rsTZB->Update();
							...
							{//4
								...
								{//3
									...
									{//2
										...
										{//1
//											rsTZB.Edit ();
											rsTZB->put_Collect((_variant_t)_T("B"),COleVariant((float)(b+delta))); 
											rsTZB->Update();
										}//1
										else
										{//1
								...
								{//3
									...
									{//2
										...
										{//1
// 											rsTZB.Edit ();
											rsTZB->put_Collect((_variant_t)_T("B"),COleVariant((float)(a+delta))); 
											rsTZB->Update();
										}//1
										else
										{//1
...
//						rsTZB.Edit();
						rsTZB->put_Collect((_variant_t)_T("B"),COleVariant((float)n)); rsTZB->Update();
					}//7
				}//8
				rsTmp->MoveNext();
			}//9
...
// 				rs.Open(dbOpenDynaset,SQLx);
				...
				//最小尺寸临时保存在拉杆size字段
// 				rsTZB.Edit(); 
				rsTZB->put_Collect((_variant_t)_T("size"),COleVariant((float)tempMinLength)); rsTZB->Update();
				rs->Close();
			}//5当前是拉杆(end)
			rsTZB->MoveNext();
		}     
...
//					rsTZB.Edit();
					...
					rsTZB->put_Collect((_variant_t)_T("sizeH"),COleVariant((double)RodLen[iNumRod]));
					//标注尺寸=实际长度-两端调整长度
					rsTZB->put_Collect((_variant_t)_T("sizeDIM"),COleVariant((long)(RodLen[iNumRod] - vtof(vTmp3)-vtof(vTmp2))));
					//拉杆作为零件计算整体单重. 单重/m由Cphs.GetPhsBHandSizes方法保存在rsTzb.Fields(_T("GDW1"))
					//gbConnectHoleDimMatch
					rsTZB->put_Collect((_variant_t)_T("L1"),COleVariant((float)RodLen[iNumRod]));
					rsTZB->put_Collect((_variant_t)_T("CLdz"),COleVariant((float)(RodLen[iNumRod] / 1000.0 * vtof(vTmp))));					
					rsTZB->put_Collect((_variant_t)_T("CLzz"),COleVariant((float)(RodLen[iNumRod] / 1000.0 * vtof(vTmp) * vtof(vTmp1))));
		  ...
		  if(dn==_T("Z2") || dn==_T("Z2A"))
		  {
			...
			else if(dn==_T("SZ5"))//固定支架SZ5按华东院坐标系计算
			{
			  ...
			  //开始华东院SZ5查找计算
				...
				//rsTmp1->put_Collect((_variant_t)_T("dw1"),vTmp);
				//rsX->get_Collect((_variant_t)_T("size3"),vTmp);
				//rsTmp1->put_Collect((_variant_t)_T("s"),vTmp);
					//先做一遍查询
					//支管外径dw,mm
					rsX->get_Collect((_variant_t)_T("sizeC"),&vTmp);
					float dw=vtof(vTmp);
...
void modPHScal::CreatePhsStructureNameAndREFIndbPRJ()
{
	...
	try
	{
		...
		{
			//这样做可以避免锁定
			EDIBgbl::dbPRJ->Execute((_bstr_t)_T("DELETE FROM PhsStructureNAME"), NULL, adCmdText);
		...
		{
			//EDIBgbl::dbPRJ->Execute((_bstr_t)_T("DROP TABLE PhsStructureREF"));			
