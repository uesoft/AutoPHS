...
CFrmBmpSe frmBmpSe;
CFrmBmpSe::CFrmBmpSe(CWnd* pParent /*=NULL*/)
	: CDialog(CFrmBmpSe::IDD, pParent)
{
	//{{AFX_DATA_INIT(CFrmBmpSe)
	m_csList1 = _T("");
	//}}AFX_DATA_INIT
//	m_Image=NULL;
//	m_imgCount=0;
	m_bDisplayFrmData=false;
	m_pDlgImgLst=new CDlgImgScroll(&FrmListBox.m_ListZDG,&this->m_List1);
}

void CFrmBmpSe::OnActivate(UINT nState, CWnd *pWndOther, BOOL bMinimized)
{
...
void CFrmBmpSe::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CFrmBmpSe)
...
BEGIN_MESSAGE_MAP(CFrmBmpSe, CDialog)
	//{{AFX_MSG_MAP(CFrmBmpSe)
	ON_WM_SHOWWINDOW()
	ON_WM_ACTIVATE()
...
/////////////////////////////////////////////////////////////////////////////
// CFrmBmpSe message handlers

BOOL CFrmBmpSe::OnInitDialog() 
{
...
LRESULT CFrmBmpSe::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
...
void CFrmBmpSe::OnImgClick(int Index)
{
  /* if (OldIndex == Index )
	   return;
   ...
   CSBmp.MovePos();
   int i;
  // Dim rsc As Recordset, rsTZB As Recordset, rsTZC As Recordset, rsTZF As Recordset
  // Dim iSEQofPart As Integer, iSEQofSPR As Integer, iChkCnd
   /*if (CSBmp.bDisplayFrmData )
   {
      FrmData.ShowWindow(SW_SHOW);
...
void CFrmBmpSe::SetHeight(int Height)
{
	CRect rc;
...
void CFrmBmpSe::SetWidth(int Width)
{
	CRect rc;
	this->GetWindowRect(&rc);
...
void CFrmBmpSe::OnShowWindow(BOOL bShow, UINT nStatus) 
{
...
void CFrmBmpSe::OnSelchangeList1() 
{
...
BEGIN_EVENTSINK_MAP(CFrmBmpSe, CDialog)
    //{{AFX_EVENTSINK_MAP(CFrmBmpSe)
	ON_EVENT(CFrmBmpSe, IDC_VSCROLLBAR, 2 /* Change */, OnChangeVscrollbar, VTS_NONE)
	ON_EVENT(CFrmBmpSe, IDC_HSCROLLBAR, 2 /* Change */, OnChangeHscrollbar, VTS_NONE)
	//}}AFX_EVENTSINK_MAP
END_EVENTSINK_MAP()

void CFrmBmpSe::OnChangeVscrollbar() 
{
	// TODO: Add your control notification handler code here
//	CSBmp.MovePos();
}

void CFrmBmpSe::OnChangeHscrollbar() 
{
	// TODO: Add your control notification handler code here
//	CSBmp.MovePos();
}

void CFrmBmpSe::RefDlgSize()
{
/*	LoadAllImage();
...
//DEL int CFrmBmpSe::incY()
//DEL {
//DEL 
//DEL }

void CFrmBmpSe::OnRButtonUp(UINT nFlags, CPoint point) 
{
...
void CFrmBmpSe::InitList()
{
	...
	for(int i=0;i<FrmListBox.m_ListZDG.GetCount();i++)
	{
		FrmListBox.m_ListZDG.GetText(i,temp);
		if(temp!="")
...
void CFrmBmpSe::OnSize(UINT nType, int cx, int cy) 
{
...
void CFrmBmpSe::OnClose() 
{
	// TODO: Add your message handler code here and/or call default
	if(IsWindow(FrmListBox.m_hWnd))
	{
		FrmListBox.m_bBmpShow=false;
		FrmListBox.UpdateData(false);
	}
	user::SavePos(this,"frmBmpSe");
	CDialog::OnClose();
}

void CFrmBmpSe::OnMenDispresult() 
{
